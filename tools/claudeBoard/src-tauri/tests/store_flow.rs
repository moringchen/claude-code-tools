...
#[test]
fn scan_refresh_removes_hook_task_when_scan_finds_no_live_pids() {
    let mut store = TaskStore::default();

    store.apply(HookEvent {
        event_type: HookEventType::PermissionRequest,
        session_id: "session-restored-active".into(),
        agent_id: None,
        pid: 903,
...
#[test]
fn dead_task_session_records_are_removed_immediately() {
    let mut store = TaskStore::default();
    ...
    assert!(persisted
        .sessions
        .iter()
        .all(|record| record.session_id != "session-dead-expired"));
}
